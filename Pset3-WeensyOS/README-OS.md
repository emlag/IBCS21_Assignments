...
virtual machine or docker. `qemu` packages are required to run WeensyOS. 
On docker, all the packages have already been installed. 

On the VM with Ubuntu, `sudo apt install qemu qemu-system-x86` should work. 


**Mac OS X:** WeensyOS can build on Mac OS X after some tools are installed.

1. run: Install [Homebrew].
2. Install Homebrew’s new GCC package. run: `brew install gcc`
3. Install Homebrew’s QEMU. run: `brew install qemu`
4. Tap [Sergio Benitez’s collection of cross-compilers](https://github.com/SergioBenitez/homebrew-osxct):
   run:  `brew tap SergioBenitez/osxct`
5. Install the `x86_64-unknown-linux-gnu` cross-compiler toolchain. run: `brew install x86_64-unknown-linux-gnu`
6. Create a file called `config.mk` in your directory and add this:
...
Computer Organization classes that has been modified by Edwin Lagos
for the IB Computer Science Resource Management Unit. 